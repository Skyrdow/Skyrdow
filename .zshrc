...
bindkey '^H' backward-kill-word     # Ctrl + Backspace (puede ser ^? o ^H según terminal)
bindkey '\e[3;5~' kill-word         # Ctrl + Suprimir (Delete)
...
function y() {
	...
	yazi "$@" --cwd-file="$tmp"
	if cwd="$(command cat -- "$tmp")" && [ -n "$cwd" ] && [ "$cwd" != "$PW:D" ]; then
		builtin cd -- "$cwd"
	fi
	rm -f -- "$tmp"
}
...
# Created by `pipx` on 2025-05-20 01:23:20
export PATH="$PATH:/home/skyrdow/.local/bin"
